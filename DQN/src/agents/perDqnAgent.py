...
from memory import PrioritiesedReplayBuffer
import pdb
import torch
...
class PerNoisyAgent():
  ...
  def __init__(self, state_size, action_size, seed, config={}, fc1_units=64, fc2_units=64):
    ...
    # Replay memory
    self.memory = PrioritiesedReplayBuffer(action_size, self.BUFFER_SIZE, self.BATCH_SIZE, seed)
  ...
  def append_samples_to_memory(self, states, actions, rewards, next_states, dones):
    ...
    next_states = torch.from_numpy(np.vstack([n for n in next_states])).float().to(device)
  ...
  def learn(self, experiences, gamma):
    ...
    # Error to update replay buffer priorities
    errors = Q_expected - Q_targets  
